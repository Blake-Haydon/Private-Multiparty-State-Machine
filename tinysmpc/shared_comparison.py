...
def _private_compare(x_sh, r, β=None):
    ...
    β_prime = any(
        ps.value == 0 for ps in d
    )  # (we break the abstraction of only operating on PrivateScalars a bit)
...
def _get_bits(n):
    ...
    return list(
        map(int, reversed(bits))
    )  # FYI: the paper requires reversed binary, but doesn't say this!
